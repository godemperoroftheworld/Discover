...
public class TextRenderManager extends DiscoverScheduler implements ClientGuiEvent.RenderHud {
    ...
    private final TextRenderer CREDITS = new TextRenderer.Builder().anchor(TextRenderer.Anchor.BOTTOM_RIGHT).centerText(false).colour(0xD3D3D3).build();
    private final TextRenderer DIMENSION = new TextRenderer.Builder().scale(3.0F).yOffset(-60).build();
    ...
    public void render(DiscoveredTitle title) {
        ...
        renderCredit(title.credit);
    }

    private void renderCredit(String credit) {
        if (CREDITS.isShowing()) {
            runInTicks(60, () -> renderCredit(credit));
        } else {
            CREDITS.setTitle(credit);
        }
    }
}
